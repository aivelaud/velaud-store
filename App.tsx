...
export default function App() {
  ...
  function install(item: AppItem) {
    if (installed.includes(item.id)) return;
    ...
    <SafeAreaView style={styles.safe}><StatusBar barStyle="light-content" />
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        ...
        {tab === 'games' && <>
          ...
          <AppRows data={games} installed={installed} downloading={downloading} onOpen={setSelected} onInstall={install} />
        </>}
        {tab === 'community' && <>
        ...
        {tab === 'library' && <>
          ...
          <View style={styles.updateBanner}><View><Text style={styles.updateTitle}>Her şey güncel</Text><Text style={styles.updateText}>Son kontrol: şimdi</Text></View><Text style={styles.check}>✓</Text></View>
          <AppRows data={catalog.filter((item) => installed.includes(item.id))} installed={installed} downloading={downloading} onOpen={setSelected} onInstall={install} library />
        </>}
        {(tab === 'discover' && query.length > 0) && <><Text style={styles.resultLabel}>Arama sonuçları</Text><AppRows data={filtered} installed={installed} downloading={downloading} onOpen={setSelected} onInstall={install} /></>}
      </ScrollView>
      ...
      <Modal visible={selected !== null} transparent animationType="slide" onRequestClose={() => setSelected(null)}>
        <View style={styles.modalShade}><View style={styles.sheet}>
          ...
          <Pressable style={[styles.install, installed.includes(selected.id) && styles.installed]} onPress={() => install(selected)}><Text style={[styles.installText, installed.includes(selected.id) && styles.installedText]}>{downloading === selected.id ? 'Yükleniyor…' : installed.includes(selected.id) ? 'Yüklü' : 'Yükle'}</Text></Pressable>
...
function AppRows({ data, installed, downloading, onOpen, onInstall, library = false }: { data: AppItem[]; installed: string[]; downloading: string | null; onOpen: (a: AppItem) => void; onInstall: (a: AppItem) => void; library?: boolean }) { return <View style={styles.rows}>{data.map((item) => <Pressable key={item.id} style={styles.row} onPress={() => onOpen(item)}><Icon item={item} /><View style={styles.rowCopy}><Text style={styles.rowName}>{item.name}</Text><Text style={styles.rowMeta}>{library ? 'Yüklü · ' : ''}{item.category} · {item.size}</Text><Text style={styles.rowScore}>★ {item.score}</Text></View><Pressable style={styles.rowAction} onPress={() => onInstall(item)}><Text style={styles.rowActionText}>{downloading === item.id ? '...' : installed.includes(item.id) ? 'Aç' : 'Yükle'}</Text></Pressable></Pressable>)}</View>; }
